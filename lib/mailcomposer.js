...
MailComposer.prototype.addHeader = function(key, value, formatted){
    ...
    if(value && Object.prototype.toString.call(value) == "[object Object]"){
        value = this._encodeMimeWord(JSON.stringify(value), "B");
    }else{
...
MailComposer.prototype._handleValue = function(key, value){
    ...
    switch(key){
        ...
        case "subject":
            value = (value || "").toString().replace(/\r?\n|\r/g, " ");
            return this._encodeMimeWord(value, "B");
    ...
    for(var i=0, len=addresses.length; i<len; i++){
        ...
        if(address.group){
            ...
            if(this._hasUTFChars(address.name)){
                address.name = this._encodeMimeWord(address.name, "B");
            }else{
...
MailComposer.prototype._createAttachmentComponent = function(attachment){
    ...
    if(attachment.fileName){
        fileName = this._encodeMimeWord(attachment.fileName, "B").replace(/"/g,"\\\"");
